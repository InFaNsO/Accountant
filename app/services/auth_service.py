...
class User(UserMixin):
    ...
    def has_permission(self, module, action):
        """action: view | create | edit | delete | financials"""
        if self.is_god():
            return True
        db = get_db()
        row = db.execute(
    ...
    def get_all_permissions(self):
        """Return dict of {module: {view,create,edit,delete[,financials]}}"""
        db = get_db()
        rows = db.execute(
        ...
        perms = {m: {"view": False, "create": False, "edit": False, "delete": False}
                 for m in MODULES}
        perms["clients"]["financials"] = False
        for r in rows:
            if r["module"] in perms:
                ...
                if r["module"] == "clients":
                    try:
                        perms["clients"]["financials"] = bool(r["can_financials"])
                    except IndexError:
                        perms["clients"]["financials"] = False
        return perms
    ...
    for module, perms in permissions.items():
        db.execute(
            """INSERT INTO user_permissions
                   (user_id, module, can_view, can_create, can_edit, can_delete, can_financials)
               VALUES (?,?,?,?,?,?,?)
